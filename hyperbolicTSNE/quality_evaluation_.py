...
def nearest_neighbor_preservation(X, Y, k_start=1, k_max=30, D_X=None, exact_nn=False, consider_order=False, strict=False, to_return="aggregated"):
    """
    Compute the nearest neighbor preservation of the of the embedding `Y`
    of the high-dimensional data `X`. See Section 6 of
    Pezzotti, Nicola, et al. "Hierarchical stochastic neighbor embedding."
    Computer Graphics Forum. Vol. 35. No. 3. 2016. for a detailed description of the approach.
    --------
    ...
    if D_X is None:
        D_X = _distance_matrix(X, method=nn_method, n_neighbors=k_max)
    D_Y = _distance_matrix(Y, method=nn_method, n_neighbors=k_max)
